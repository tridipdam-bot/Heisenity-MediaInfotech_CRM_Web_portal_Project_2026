...
export class LeaveService {
    ...
    async getEmployeeLeaveApplications(employeeId) {
        try {
            const employee = await prisma.employee.findUnique({
                where: { employeeId }
            });
            ...
            const response = leaveApplications.map(app => ({
                id: app.id,
    ...
    async reviewLeaveApplication(data) {
        try {
    ...
    async cancelLeaveApplication(applicationId, employeeId) {
        try {
            const employee = await prisma.employee.findUnique({
                where: { employeeId }
            });
