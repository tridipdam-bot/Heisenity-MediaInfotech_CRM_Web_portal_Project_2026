...
export class EmployeeIdGeneratorService {
   ...
   */
  static async generateNextLegacyEmployeeId(): Promise<string> {
    try {
   ...
   */
  static validateEmployeeIdFormat(employeeId: string, role?: 'FIELD_ENGINEER' | 'IN_OFFICE'): boolean {
   ...
   */
  static async getNextAvailableIds(role: 'FIELD_ENGINEER' | 'IN_OFFICE' = 'FIELD_ENGINEER', count: number = 5): Promise<string[]> {
    try {
